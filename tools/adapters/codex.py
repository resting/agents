...
- `.codex/skills/<plugin>__<skill>/SKILL.md` (8 KB body cap; overflow into references/) — gitignored
- `.codex/agents/<plugin>__<agent>.toml` (markdown YAML -> TOML transform) — gitignored
- `plugins/<plugin>/.codex-plugin/plugin.json` — committed per-plugin manifest; the Codex
  marketplace installs the source plugin dir directly (reads `SKILL.md` from `./skills/`).
- `.agents/plugins/marketplace.json` — committed Codex marketplace registry listing every plugin.

`AGENTS.md` is NOT generated by this adapter — it's the committed cross-harness
context file at the repo root (read directly by Codex/Cursor/OpenCode/Antigravity CLI;
Claude Code reads it via `CLAUDE.md`, a symlink to `AGENTS.md`). `emit_global` validates
that file fits Codex's 32 KiB cap and emits the marketplace registry.
...
class CodexAdapter(HarnessAdapter):
    ...
    def emit_plugin(self, plugin: PluginSource) -> EmitResult:
        ...
        # Per-plugin Codex manifest for native marketplace install (reads source skills).
        self._emit_codex_plugin_manifest(plugin, result)

        return result

    def emit_global(self, plugins: list[PluginSource]) -> EmitResult:
        """Validate the committed root AGENTS.md and emit the Codex marketplace registry.
        ...
        by Codex / Cursor / OpenCode / Antigravity CLI; Claude Code reads it via
        `CLAUDE.md`, a symlink to `AGENTS.md`). The adapter validates that the committed
        file fits Codex's constraints (32 KiB hard cap, ~150-line table-of-contents
        convention) and writes `.agents/plugins/marketplace.json`.
        ...
        if not agents_md.is_file():
            result.warnings.append(
                ...
                "context file for Codex/Cursor/OpenCode/Antigravity CLI; Claude Code reads "
                "it via `CLAUDE.md`, a symlink to `AGENTS.md`."
            )
        else:
        ...
        # Codex marketplace registry: lists every local plugin so each installs
        # individually. Lives at the project-level `.agents/plugins/marketplace.json`
        # (auto-discovered when the repo is cwd; also addable via
        # `npx codex-marketplace add <repo>`). Entries point at the per-plugin bundles
        # emitted under `.codex/plugins/<plugin>/`.
        result.written.append(
            self.write(
                Path(".agents") / "plugins" / "marketplace.json",
                json.dumps(self._codex_marketplace(plugins), indent=2) + "\n",
            )
        )
        return result
    ...
    def _emit_codex_plugin_manifest(self, plugin: PluginSource, result: EmitResult) -> None:
        ...
        (`source.path: ./plugins/<plugin>`), so no transformed skill tree is committed —
        Codex reads `SKILL.md` natively from `plugins/<plugin>/skills/` (`skills: "./skills/"`),
        exactly as the Cursor marketplace does. This keeps the repo lean: only the tiny
        manifest is committed, not a duplicated skill tree. Skills above Codex's 8 KB
        injection cap are truncated by Codex at load; the transformed, cap-split copies
        under `.codex/skills/` (gitignored) remain for the `~/.codex/skills` symlink recipe.
        """
        manifest_path = Path("plugins") / plugin.name / ".codex-plugin" / "plugin.json"
        result.written.append(
            self.write(
                manifest_path,
                json.dumps(self._codex_plugin_manifest(plugin), indent=2) + "\n",
            )
        )
    ...
    def _read_marketplace_root(self) -> dict:
        """Read the source `.claude-plugin/marketplace.json` for name/owner/metadata."""
        path = self.repo_root / ".claude-plugin" / "marketplace.json"
        if not path.is_file():
            return {}
        try:
